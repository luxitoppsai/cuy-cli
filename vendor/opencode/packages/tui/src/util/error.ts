...
export function cliErrorMessage(input: unknown): string | undefined {
  ...
  if (model) {
    ...
    return [
      ...
      "Try: `cuy models` to list available models",
      "Or check your cuycli configuration provider/model names",
    ].join("\n")
  }
  ...
  if (remoteAuth) {
    ...
    return [
      ...
      ...(url ? [`Run \`cuy auth login ${url}\` to re-authenticate.`] : []),
    ].join("\n")
  }
  ...
  if (isRecord(input) && named(input, "MCPFailed")) {
    ...
    return `MCP server "${name}" failed. Note, cuycli does not support MCP authentication yet.`
  }
  return undefined
}
