      ...
      <box width={contentWidth()} flexGrow={1} flexDirection="column" paddingTop={1} paddingBottom={1} gap={1}>
        ...
        <Show when={showFooter()}>
          <box flexDirection="column" alignItems="center" flexShrink={0}>
            ...
            <text fg={colors.muted}>cuycli {InstallationVersion}</text>
          </box>
        </Show>
      </box>
    </box>
...
function buildIssueURL(message: string, stack: string) {
  ...
  const url = new URL("https://github.com/luxitoppsai/cuy-cli/issues/new")
  ...
  url.searchParams.set(
    "reproduce",
    "Reported automatically from the cuycli crash screen. If you can, describe what you were doing when it crashed.",
  )
  ...
  const marker = "\n… (truncated)"
  const head = `The cuycli TUI crashed with an unexpected error.\n\n**Error:** ${message}\n\n**Stack trace:**\n`
